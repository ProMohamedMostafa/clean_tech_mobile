...
Future<void> createMaterialPDF(BuildContext context) async {
  final cubit = context.read<MaterialManagementCubit>();
  final users = cubit.materialManagementModel?.data?.materials ?? [];
  ...
  for (final user in users) {
    ...
    row.cells[1].value = user.categoryName ?? '';
    row.cells[2].value = user.quantity.toString();
    row.cells[3].value = user.minThreshold.toString();
    row.cells[4].value = user.unit ?? '';
  }
  ...
  final fileName = 'materials_${DateTime.now().millisecondsSinceEpoch}.pdf';
