...
Future<void> createCategoryPDF(BuildContext context) async {
  final cubit = context.read<CategoryManagementCubit>();
  final users = cubit.categoryManagementModel?.data?.categories ?? [];
  ...
  header.cells[2].value = 'Parent Category Name';

  // ======= Dynamic Rows from API =======
  for (final user in users) {
    final row = grid.rows.add();
    row.cells[0].value = user.name ?? '';
    row.cells[1].value = user.unit ?? '';
    row.cells[2].value = user.parentCategoryName ?? '';
  }
  ...
  final fileName = 'categories_${DateTime.now().millisecondsSinceEpoch}.pdf';
