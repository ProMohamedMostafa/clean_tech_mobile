...
Future<void> createTransactionPDF(BuildContext context) async {
  final cubit = context.read<TransactionManagementCubit>();
  final users = cubit.transactionManagementModel?.data?.data ?? [];
  ...
  const String title = "Transaction";
  ...
  header.cells[3].value = 'Provider';
  header.cells[4].value = 'Created By';
  header.cells[5].value = 'Unit';
  header.cells[6].value = 'Type';

  // ======= Dynamic Rows from API =======
  for (final user in users) {
    final row = grid.rows.add();
    row.cells[0].value = user.name ?? '';
    row.cells[1].value = user.category ?? '';
    row.cells[2].value = user.quantity.toString();
    row.cells[3].value = user.provider ?? '';
    row.cells[4].value = user.userName ?? '';
    row.cells[5].value = user.unitName ?? '';
    row.cells[6].value = user.type ?? '';
  }
  ...
  final fileName = 'transaction_${DateTime.now().millisecondsSinceEpoch}.pdf';
