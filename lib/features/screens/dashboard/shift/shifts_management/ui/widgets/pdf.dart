...
Future<void> createShiftPDF(BuildContext context) async {
  final cubit = context.read<ShiftCubit>();
  final users = cubit.allShiftsModel?.data?.data ?? [];
  ...
  header.cells[1].value = 'Start Date';
  header.cells[2].value = 'End Date';
  ...
  for (final user in users) {
    ...
    row.cells[1].value = user.startDate ?? '';
    row.cells[2].value = user.endDate ?? '';
    row.cells[3].value = user.startTime ?? '';
    row.cells[4].value = user.endTime ?? '';
  }
  ...
  final fileName = 'shifts_${DateTime.now().millisecondsSinceEpoch}.pdf';
