...
class DeviceAnswersBody extends StatelessWidget {
  final int id;
  const DeviceAnswersBody({super.key, required this.id});

  @override
  Widget build(BuildContext context) {
    final cubit = context.read<DeviceAnswersCubit>();
    return BlocBuilder<DeviceAnswersCubit, DeviceAnswersState>(
      builder: (context, state) {
        final data = cubit.devicesModel?.data?.data;
        final bool isFilterActive = cubit.filterModel != null;

        return Scaffold(
          appBar: AppBar(
            title: Text(S.of(context).device_answers),
            actions: [
              Padding(
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return InkWell(
                      ...
                      onTap: () {
                        context.pushNamed(Routes.feedbackAnswerScreen,
                            arguments: item.id);
                      },
                      child: Container(
                        decoration: BoxDecoration(
