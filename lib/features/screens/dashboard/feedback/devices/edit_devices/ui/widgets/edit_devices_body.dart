...
class EditDevicesBody extends StatelessWidget {
  final int id;
  const EditDevicesBody({super.key, required this.id});

  @override
  Widget build(BuildContext context) {
    final cubit = context.read<EditDevicesCubit>();
    return Scaffold(
      appBar: AppBar(leading: CustomBackButton(), title: Text(S.of(context).edit_device)),
      body: BlocConsumer<EditDevicesCubit, EditDevicesState>(
        listener: (context, state) {
          if (state is EditDeviceSuccessState) {
            toast(text: state.message, isSuccess: true);
            context.popWithTrueResult();
          }
          if (state is EditDeviceErrorState) {
            toast(text: state.error, isSuccess: false);
          }
        },
        builder: (context, state) {
          if (cubit.buildingModel?.data! == null ||
              cubit.deviceDetailsModel?.data! == null) {
            return Loading();
          }
                                ...
                                children: [
                                  Expanded(
                                    ...
                                    children: [
                                      ...
                                      verticalSpace(5),
                                      CustomDropDownList(
                                        hint: cubit.deviceDetailsModel!.data!
                                                .buildingName ??
                                  ...
                                  verticalSpace(20),
                                  state is EditDeviceLoadingState
                                      ? Loading()
                                      : Center(
