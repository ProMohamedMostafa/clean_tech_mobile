...
Future<void> createAttendanceHistoryPDF(BuildContext context) async {
  final cubit = context.read<AttendanceHistoryCubit>();
  final users = cubit.attendanceHistoryModel?.data?.data ?? [];
  ...
  header.cells[1].value = 'Role';
  header.cells[2].value = 'Clock In';
  header.cells[3].value = 'Clock Out';
  ...
  for (final user in users) {
    ...
    row.cells[1].value = user.role ?? '';
    row.cells[2].value = user.clockIn != null
        ? DateFormat('hh:mm a').format(DateTime.parse(user.clockIn!))
        : '';
    row.cells[3].value = user.clockOut != null
        ? DateFormat('hh:mm a').format(DateTime.parse(user.clockOut!))
        : '';
    row.cells[4].value = cubit.formatDuration(user.duration ?? '');
    row.cells[5].value = user.status ?? '';
  }
  ...
  final fileName = 'attendances_${DateTime.now().millisecondsSinceEpoch}.pdf';
