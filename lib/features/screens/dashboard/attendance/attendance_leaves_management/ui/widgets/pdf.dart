...
Future<void> createLeavesPDF(BuildContext context) async {
  final cubit = context.read<AttendanceLeavesCubit>();
  final users = cubit.allLeaves?.data?.leaves ?? [];
  ...
  for (final user in users) {
    ...
    row.cells[2].value = user.startDate ?? '';
    row.cells[3].value = user.endDate ?? '';
    row.cells[4].value = user.type;
  ...
  final fileName = 'leaves_${DateTime.now().millisecondsSinceEpoch}.pdf';
