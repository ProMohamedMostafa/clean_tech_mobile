...
import 'package:smart_cleaning_application/features/screens/dashboard/sensor/sensor_edit/data/model/edit_sensor.dart';
...
part 'edit_sensor_state.dart';

class EditSensorCubit extends Cubit<EditSensorState> {
  EditSensorCubit() : super(EditSensorInitial());
  ...
  editSensor(int? id, {bool removeLocation = false}) async {
    emit(EditSensorLoadingState());
    try {
      ...
      final editSensorModel = EditSensorModel.fromJson(response!.data);
      emit(EditSensorSuccessState(editSensorModel));
    } catch (error) {
      emit(EditSensorErrorState(error.toString()));
    }
  }
