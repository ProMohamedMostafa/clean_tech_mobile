...
class CompleteTasksBarChart extends StatelessWidget {
  final CompletetionTaskModel? completeTaskData;
  const CompleteTasksBarChart({super.key, this.completeTaskData});
            ...
            series: <CartesianSeries>[
              ColumnSeries<ChartData, String>(
                ...
                dataSource: List.generate(
                  completeTaskData?.data?.labels?.length ?? 0,
                  (index) => ChartData(
                    completeTaskData?.data?.labels?[index] ?? '',
                    completeTaskData?.data?.values?[index] ?? 0,
                  ),
                ),
