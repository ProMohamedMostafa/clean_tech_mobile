...
class AddTaskCubit extends Cubit<AddTaskState> {
  ...
  addTask(int priorityId, int? statusId, int? buildingId, int? floorId,
      int? pointId,List<int>? selectedSupervisorIds) async {
    ...
    Map<String, dynamic> formDataMap = {
      ...
      "Files": MultipartFile.fromFile(image!.path),
    };
    ...
    try {
      ...
      emit(AddTaskErrorState(createTaskModel!));
    }
  }

  CleanerModel? cleanerModel;
  getCleaner(int id) {
    emit(GetCleanerLoadingState());
    DioHelper.getData(url: 'Cleaners//$id').then((value) {
      // cleanerModel = CleanerModel.fromJson(value!.data);
      emit(GetCleanerSuccessState(cleanerModel!));
    }).catchError((error) {
      emit(GetCleanerErrorState(error.toString()));
    });
  }
